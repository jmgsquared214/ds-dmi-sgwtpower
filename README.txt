Direct Method Invocation (DMI) DataSource
-----------------------------------------
This example illustrates setting up a DataSource that calls methods on your
configured server bean in response to DataSource operations (fetch, add,
update, remove).
...
- 'ant war'
  Compile for deployment and bundle into DataSourceDMI.war file.

  If you have a web container operating on localhost:8080, you can deploy
  DataSourceDMI.war into the web container and access it at:

    http://localhost:8080/DataSourceDMI/DataSourceDMI.html
